...
const create_line = (x0: number, y0: number, x1: number, y1: number) =>
	new fabric.Line([x0 + VARS.rw, y0 + VARS.rh / 2, x1, y1 + VARS.rh / 2], {
		fill: COLOR.line,
		stroke: COLOR.line,
...
function add_prereq(curClass: string, toAdd: string, canvas: any) {
	...
	if (!node_exists(toAdd)) {
		if (cur.c === 0) SPACES.shift_fwd(spaces, allNodes, canvas);
...
function add_goal(
	curClass: string,
	goal: string,
	prereqsDirectory: { [name: string]: Array<string> },
	canvas: any
) {
	add_orphan(curClass, canvas);
	if (!prereqsDirectory.curClass || prereqsDirectory.curClass.length === 0)
		return;
